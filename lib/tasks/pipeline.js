...
module.exports = Task.extend({
  ...
  _aliasHash: function(deployConfig, plugins) {
    ...
    for(var i=0, len=aliasList.length; i<len; i++) {
      ...
      var configuredName = split[1] ? alias.match(configuredNameRegex)[2] : split[0];

      aliasHash[configuredName] = plugins[pluginName];

      if(!aliasHash[configuredName]) {
        throw new SilentError('Configured alias `' + configuredName +'` for plugin `' + pluginName +'`, which is not available.');
      }
    }
