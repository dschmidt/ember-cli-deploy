...
module.exports = Task.extend({
  ...
  _registerPipelineHooks: function(addon, deployConfig) {
    ...
    var configuredNameRegex = /(.*?):(.*)/;
    var configuredPluginHash = Object.create(null);
    (deployConfig.plugins || [name]).forEach(function (pluginName) {
      var split = pluginName.split(':');
      if(split[0] === name) {
        if(split[1]) {
          configuredPluginHash[pluginName.match(configuredNameRegex)[2]] = split[0];
        } else {
          configuredPluginHash[split[0]] = split[0];
        }
      }
    });

    Object.keys(configuredPluginHash).forEach(function(configuredName) {
      if(configuredPluginHash[configuredName] === name) {
        self._createDeployPluginsForConfiguredName(addon, name, configuredName);
      }
    });
  },

  _createDeployPluginsForConfiguredName: function(addon, pluginName, configuredName) {
    var deployPlugin = addon.createDeployPlugin({name: configuredName});
