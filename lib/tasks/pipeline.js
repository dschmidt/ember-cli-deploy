...
module.exports = Task.extend({
  ...
  _registerPipelineHooks: function(addon, deployConfig) {
    var self = this;
    var configuredPluginList = deployConfig.plugins.map(function (pluginName) {
    ...
    deployConfig.plugins.forEach(function (pluginName) {
      ...
      if(split[0] === name) {
        if(split[1]) {
          configuredNames.push(pluginName.match(configuredNameRegex)[2]);
        } else {
          configuredNames.push(split[0]);
        }
      }
    });

    configuredNames.forEach(function(configuredName) {
